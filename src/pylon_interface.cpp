  ...
  switch (cam_type_)
  {
    case GIGE:
      try
      {
        ...
        {
          cout << "Error: " << ptr_grab_result_->GetErrorCode() << " " << ptr_grab_result_->GetErrorDescription()
               << endl;
        }
      }
...
int PylonInterface::setupCameraConfiguration(const PylonCameraParameter &params)
{
  switch (cam_type_)
  {
    case GIGE:
      try
      {
        gige_cam_->RegisterConfiguration(new CSoftwareTriggerConfiguration, RegistrationMode_ReplaceAll,
                                         Cleanup_Delete);
        gige_cam_->Open();
    ...
    case DART:
      try
      {
        dart_cam_->RegisterConfiguration(new CSoftwareTriggerConfiguration, RegistrationMode_ReplaceAll,
                                         Cleanup_Delete);
        dart_cam_->Open();
        this->initSequencer(params);
...
int PylonInterface::findDesiredCam(const PylonCameraParameter &params)
{
  ...
  { // Select camera device with specified magazino_cam_id
    ...
    {
      cerr << "An exception while opening the desired camera with Magazino ID: " << params.magazino_cam_id_
           << " occurred:" << endl;
      cerr << e.GetDescription() << endl;
      return 3;
    }
        ...
        else
        {
          ...
          if (gige_cam_->AutoTargetValue.GetMin() > brightness)
          {
            cout << "Desired brightness (" << brightness << ") unreachable! Setting to lower limit: "
                 << gige_cam_->AutoTargetValue.GetMin() << endl;
          ...
          else if (gige_cam_->AutoTargetValue.GetMax() < brightness)
          {
            cout << "Desired brightness (" << brightness << ") unreachable! Setting to upper limit: "
                 << gige_cam_->AutoTargetValue.GetMax() << endl;
