...
int PylonCameraOpenCVNode::init()
{
    ...
    exp_times_pub_ = new ros::Publisher(nh_.advertise<pylon_camera_msgs::SequenceExposureTimes>("seq_exp_times",
                                                                                                10));
    ...
    if (calib_loader_->img_cols_ != img_raw_msg_.width || calib_loader_->img_rows_
                                                          != img_raw_msg_.height)
    {
        cerr << "Error: Image size from yaml file (" << calib_loader_->img_cols_ << ", "
             << calib_loader_->img_rows_
    ...
    setupCameraInfoMsg();
    img_rectifier_.setupRectifyingMap(calib_loader_->K_,
                                      calib_loader_->D_,
...
//            pylon_interface_->usb_cam_->SequencerSetSelector.SetValue(i);
            cv::Mat img(pylon_interface_->img_rows(), pylon_interface_->img_cols(),
            CV_8UC1);
        ...
        cv_img_seq_.image = cv::Mat(pylon_interface_->img_rows(), pylon_interface_->img_cols(),
        CV_8UC3);
