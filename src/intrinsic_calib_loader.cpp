...
IntrinsicCalibLoader::IntrinsicCalibLoader(const std::string yaml_file) :
		img_cols_(-1), img_rows_(-1), K_(), D_(), intrinsic_yaml_file_("INVALID_YAML_FILE") {
...
bool IntrinsicCalibLoader::loadCalib() {
	...
	if (!fs.isOpened()) {
		...
		if (img_cols_ <= 0 || img_cols_ <= 0) {
			cerr << "Error: Unexpected img size (" << img_cols_ << ", " << img_rows_ << ") while reading from yaml file!" << endl;
		}
		if (D_.rows != 5 && D_.cols != 1) {
			cerr << "Error: Unexpected size of distortion matrix (" << D_.rows << " x " << D_.cols << ") while reading from yaml file! Should be: (5 x 1)" << endl;
		}
		if (K_.rows != 3 && K_.cols != 3) {
			cerr << "Error: Unexpected size of camera matrix (" << K_.rows << " x " << K_.cols << ") while reading from yaml file! Should be: (3 x 3)" << endl;
		}
	}
